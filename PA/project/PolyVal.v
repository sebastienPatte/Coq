(* In this file, we prove that that sum and multiplication are morphism for polynom evaluation *)
...
Theorem val_coeff (p q : valid_poly) : (forall (f:nat -> Z), poly_val p f = poly_val q f) -> (forall (m: monoid ), get_coefficient p m = get_coefficient q m).
Proof.
  intros.
  destruct p as [p ].
  destruct q as [q ].
...
Theorem sum_poly_val (p q : valid_poly) (val:nat->Z) : 
    ...
    + valid_destr VQ.
      ...
      * elim a; intro.
        -- apply nat_compare_gt in a0 as a1.
          rewrite a1.
          rewrite sum_pol_comm; try assumption.
          simpl.
          rewrite IHq1.
          lia.
...
*)
Theorem mul_poly_val (p q : valid_poly) (val:nat->Z) : 
