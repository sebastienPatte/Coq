...
Require Import Prelude Bool List Lia Program Nat Arith (* Omega *).
...
Proof.
    ...
    -   s_all apply IHl. concat_Forall_app in H;  destruct H as [_ ?].
...
Proof.
    ...
    destruct f; try destruct a; Forall_sum_map.
    repeat (concat_Forall_app; saut split).

    
    destruct f; destruct a; constructors_Forall; sum_map_app. 
    1-24: lia.
    1-7: simpl; auto.
    
    destruct a; constructors_Forall; simpl; auto.
1-5: try rewrite map_app; try rewrite list_sum_app; simpl; auto; lia.
    simpl in IHl.
	rewrite pick_hyp_prepend.
	
	 rewrite map_map.
    concat_Forall_app in IHl.
    destruct IHl as [_ ?].
    rewrite <- plus_assoc. 
	now apply Forall_concat_prepend.
Qed.
